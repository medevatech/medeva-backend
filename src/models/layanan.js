  ...
  return new Promise((resolve, reject) =>
    Pool.query(
      `INSERT INTO tbl_layanan 
        (id, id_kunjungan, id_daftar_layanan, id_pasien, catatan, status, is_active,
...
const allLayanan = ({
  ...
  return new Promise((resolve, reject) =>
    Pool.query(
      ...
      AND
        tbl_layanan.status ILIKE '%${searchStatus}%'
      AND
        CAST(tbl_layanan.is_active AS TEXT) ILIKE '%${searchActive}%'
...
const editLayanan = (data) => {
  ...
  return new Promise((resolve, reject) =>
    Pool.query(
      `UPDATE tbl_layanan 
      SET
        id_kunjungan='${id_kunjungan}', id_daftar_layanan='${id_daftar_layanan}', id_pasien='${id_pasien}', catatan='${catatan}', status='${status}',  is_active=${is_active},
        updated_at=NOW()
      WHERE id='${id}'`,
      (err, result) => {
        if (!err) {
